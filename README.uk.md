...
1. **[⬇ Завантажити ZIP — v0.4.0](https://github.com/Zapadenec1982/fpv-fm-emulator/archive/refs/tags/v0.4.0.zip)** — версія з
...
tests/            офлайн-тести ядра (pytest, 171 шт.)
```
...
завертається лише 0.34 % потужності. Стеля — сама плата: цей Pluto+ тримає максимум
**30.72 MSPS** (40 / 50 / 61.44 відхиляються з errno 22, і вимкнення TX FIR цього не
змінює), тож найширше практичне рознесення — близько 20 МГц. Для більшого потрібен
другий SDR на власній несучій. Ближчі зсуви дають попередження, а не помилку.
...
```
171 офлайн-тест, апаратура не потрібна. Покривають: пошук бендів/каналів (зокрема
