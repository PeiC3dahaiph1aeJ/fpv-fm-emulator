...
tests/            офлайн-тести ядра (pytest, 121 шт.)
```
...
- **Мова:** перемикач у панелі **«Вихід»** змінює мову інтерфейсу
  (English / Українська); вибір запам'ятовується між запусками.
...
```
121 офлайн-тест, апаратура не потрібна. Покривають: пошук бендів/каналів (зокрема
...
каталог покриває кожен рядок, який показують код і штатні YAML.
