...
CATALOG_UK: Dict[str, str] = {
    "This .venv was built by a different Python than the one running it — which is what happens when the project folder is copied between computers with .venv inside. It cannot be copied: it records the path of the Python that made it and holds binaries compiled for that exact version. Delete the .venv folder and run setup.bat on this computer.": "Це .venv створено іншим Python, ніж той, що його запускає — так буває, коли папку проєкту копіюють між компʼютерами разом із .venv. Копіювати його не можна: воно памʼятає шлях до Python, який його створив, і містить бінарники, скомпільовані саме під ту версію. Видаліть папку .venv і запустіть setup.bat на цьому компʼютері.",
