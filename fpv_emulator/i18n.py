...
CATALOG_UK: Dict[str, str] = {
    ...
    "Firmware: {name} ({version})": "Прошивка: {name} ({version})",
    "Firmware: {version} (not recognised)": "Прошивка: {version} (не розпізнано)",
    "Auto tries the filtered path first and falls back if the board refuses it — right for both boards here. Pick a firmware only to force one route: Tezuka skips the attempt, Analog Devices refuses to fall back.": "«Авто» спершу пробує шлях із фільтром і відкочується, якщо плата його не приймає — це правильний вибір для обох наших плат. Обирайте прошивку лише щоб примусово задати маршрут: «Tezuka» пропускає спробу, «Analog Devices» відмовляється відкочуватись.",
    "The firmware profile is set to {chosen}, but this board reports {actual} ({version}). The profile is being applied as you selected it — set it to {auto} to let the board decide.": "Профіль прошивки задано як {chosen}, але ця плата повідомляє {actual} ({version}). Профіль застосовано так, як ви його обрали — поставте {auto}, щоб рішення приймала сама плата.",
    "The {profile} firmware profile does not set the rate any other way. Switch the profile to {auto} to let the board be asked without the FIR filter.": "Профіль прошивки {profile} не встановлює частоту жодним іншим способом. Перемкніть профіль на {auto}, щоб плату запитали без FIR-фільтра.",
    "The {profile} firmware profile set {rate} MSPS with the FIR filter off, without trying the filtered path first. Image rejection either side of the passband is a little weaker — the video itself is unchanged.": "Профіль прошивки {profile} встановив {rate} MSPS із вимкненим FIR-фільтром, навіть не пробуючи спершу шлях із фільтром. Придушення дзеркал обабіч смуги трохи гірше — саме відео не змінюється.",
    "Setting the sample rate to {rate} MSPS with the FIR filter off did not take (firmware profile: {profile}).{avail}": "Встановити частоту дискретизації {rate} MSPS із вимкненим FIR-фільтром не вдалося (профіль прошивки: {profile}).{avail}",
    "[info] firmware profile forced to «{name}» — it stays set for the next launch too. Auto is right for both boards.": "[info] профіль прошивки примусово встановлено на «{name}» — він лишиться таким і на наступний запуск. Для обох наших плат правильний вибір — «Авто».",
    "[info] firmware profile: {name} (not auto)": "[info] профіль прошивки: {name} (не «Авто»)",
    "firmware profile: which ways of setting the sample rate may be tried (default: auto — try the filtered path, fall back)": "профіль прошивки: які способи задання частоти дискретизації дозволено пробувати (типово: auto — спробувати шлях із фільтром, за потреби відкотитись)",
    ...
    "single carrier, MHz (ignored when --freq-list is given)": "одна несуча, МГц (ігнорується, якщо задано --freq-list)",
    "tx() succeeded but the TX buffer is not playing (attempt {n})": "tx() пройшов, але TX-буфер не програється (спроба {n})",
